...
public class Server {
    ...
    /**
     * Ajoute le 'server.EventHandler' spécifié aux 'handlers'.
     *
    ...
    public void handleEvents(String cmd, String arg) {
        if (cmd.equals(REGISTER_COMMAND)) { // Si le client demande de s'incrire à un cours
    ...
    public void handleLoadCourses(String arg) {
        try {
            // Scanner et inputStream du fichier de la liste de cours
            Scanner scanCours = new Scanner(new File(PATH_LISTE_COURS));
            ObjectInputStream courseInputStream = new ObjectInputStream(new FileInputStream(PATH_LISTE_COURS));
            // Liste qui sera retournée
            List<Course> coursDeLaSession = null;

            while (scanCours.hasNext()) { // Tant qu'il reste des lignes à lire dans le fichier
                scanCours.nextLine(); // Passer à la prochaine ligne
                Course cours = (Course) courseInputStream.readObject(); // Lire un cours

                // Filtrer les cours selon la session
                if (cours.getSession().equalsIgnoreCase(arg)) // S'il est de la session demandée par le client
                    coursDeLaSession.add(cours); // L'ajouter à la liste
            }
            ...
            System.out.println("Il est impossible de lire 'Course' dans le fichier de la liste des cours.");
